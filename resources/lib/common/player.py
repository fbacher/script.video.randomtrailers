...
from rt_utils import Monitor
from rt_constants import Constants
...
class BasePlayerHandler(object):
    def __init__(self, player, session_id=None):
        ...
        self._logger = Logger(Constants.addonName)
...
class SeekPlayerHandler(BasePlayerHandler):
    ...
    def setup(self, duration, offset, bif_url, title='', title2='', seeking=NO_SEEK):
        ...
        self.dialog.setup(self.duration, int(
            self.baseOffset * 1000), self.bifURL, self.title, self.title2)

    def getDialog(self, setup=False):
        if not self.dialog:
            pass
        #    self.dialog = seekdialog.SeekDialog.create(
        #        show=False, handler=self)
    ...
    def showPostPlay(self):
        ...
        self.player.trigger('post.play', video=self.player.video,
                            playlist=self.playlist, handler=self)
    ...
    def rewind(self):
        if self.mode == self.MODE_ABSOLUTE:
            ...
            self.seek(max(self.trueTime - 30, 0) *
                      1000, seeking=self.SEEK_REWIND)
    ...
    def onPlayBackStarted(self):
        Logger.debug(
            'SeekHandler: onPlayBackStarted - mode={0}'.format(self.mode))
    ...
    def onPlayBackStopped(self):
        Logger.debug(
            'SeekHandler: onPlayBackStopped - Seeking={0}'.format(self.seeking))
    ...
    def onPlayBackEnded(self):
        Logger.debug(
            'SeekHandler: onPlayBackEnded - Seeking={0}'.format(self.seeking))
        ...
        if self.player.playerObject.hasMoreParts():
            ...
            self.player._playVideo(
                self.player.playerObject.getNextPartOffset(), seeking=self.seeking)
            return
    ...
    def setSubtitles(self):
        ...
        if subs:
            ...
            if path:
                if self.mode == self.MODE_ABSOLUTE:
                    Logger.debug('Setting subtitle path: {0}'.format(path))
                ...
                if self.mode == self.MODE_ABSOLUTE:
                    Logger.debug(
                        'Enabling embedded subtitles at: {0}'.format(subs.typeIndex))
                    Logger.debug('Kodi reported subtitles: {0}'.format(
            ...
            if track:
                try:
                    currIdx = kodijsonrpc.rpc.Player.GetProperties(
                        playerid=1, properties=['currentaudiostream'])['currentaudiostream']['index']
                    if currIdx == track.typeIndex:
                        Logger.debug(
                            'Audio track is correct index: {0}'.format(track.typeIndex))
                        return
                except:
                    Logger.error()
                ...
                xbmc.sleep(100)
                Logger.debug(
                    'Switching audio track - index: {0}'.format(track.typeIndex))
    ...
    def onPlayBackFailed(self):
        Logger.debug(
            'SeekHandler: onPlayBackFailed - Seeking={0}'.format(self.seeking))
        if self.seeking not in (self.SEEK_IN_PROGRESS, self.SEEK_PLAYLIST):
            self.sessionEnded()
    ...
    def onVideoWindowClosed(self):
        self.hideOSD()
        Logger.debug(
            'SeekHandler: onVideoWindowClosed - Seeking={0}'.format(self.seeking))
        if not self.seeking:
            self.player.stop()
    ...
    def sessionEnded(self):
        if self.ended:
            return
        self.ended = True
        Logger.debug('Player: Video session ended')
...
class AudioPlayerHandler(BasePlayerHandler):
    ...
    def extractTrackInfo(self):
        ...
        for x in range(10):  # Wait a sec (if necessary) for this to become available
            try:
                item = kodijsonrpc.rpc.Player.GetItem(
                    playerid=0, properties=['comment'])['item']
                plexID = item['comment']
            except:
                Logger.error()
        ...
        Logger.debug('Extracting track info from comment')
        try:
    ...
    def playQueueCallback(self, **kwargs):
        ...
        # plist.clear()
        try:
            citem = kodijsonrpc.rpc.Player.GetItem(
                playerid=0, properties=['comment'])['item']
        ...
        for x in range(size - 1, current, -1):  # First everything with a greater position
            kodijsonrpc.rpc.Playlist.Remove(
                playlistid=xbmc.PLAYLIST_MUSIC, position=x)
        for x in range(current):  # Then anything with a lesser position
            kodijsonrpc.rpc.Playlist.Remove(
                playlistid=xbmc.PLAYLIST_MUSIC, position=0)
        ...
        # Now swap the track to the correct position. This seems to be the only
        # way to update the kodi playlist position to the current track's new
        # position
        if swap is not None:
            kodijsonrpc.rpc.Playlist.Swap(
                playlistid=xbmc.PLAYLIST_MUSIC, position1=0, position2=swap + 1)
            kodijsonrpc.rpc.Playlist.Remove(
                playlistid=xbmc.PLAYLIST_MUSIC, position=0)
...
class AdvancedPlayer(xbmc.Player):
    ...
    def control(self, cmd):
        if cmd == 'play':
            Logger.debug('Player - Control:  Command=Play')
            if xbmc.getCondVisibility('Player.Paused | !Player.Playing'):
                Logger.debug('Player - Control:  Playing')
                xbmc.executebuiltin('PlayerControl(Play)')
        elif cmd == 'pause':
            Logger.debug('Player - Control:  Command=Pause')
            if not xbmc.getCondVisibility('Player.Paused'):
                Logger.debug('Player - Control:  Pausing')
                xbmc.executebuiltin('PlayerControl(Play)')
    ...
    def playAt(self, path, ms):
        ...
        kodijsonrpc.rpc.Player.Open(
            item={'file': path},
            options={'resume': {'hours': h, 'minutes': m,
    ...
    def playVideo(self, video, resume=False, force_update=False, session_id=None, handler=None):
        ...
        self.video = video
        self.open()
        self._playVideo(resume and video.viewOffset.asInt()
                        or 0, force_update=force_update)
    ...
    def _playVideo(self, offset=0, seeking=0, force_update=False, playerObject=None):
        ...
        self.trigger(
            ...
            url=self.video.defaultArt.asTranscodedImageURL(
                1920, 1080, opacity=60)  # , background=colors.noAlpha.Background)
        )

        # Stop before setting up the handler to prevent player events from
        # causing havoc
        self.stopAndWait()

        self.handler.setup(self.video.duration.asInt(), offset, u'',
                           title=self.video.grandparentTitle, title2=self.video.title, seeking=seeking)

        li = xbmcgui.ListItem(self.video.title)
    ...
    def playVideoPlaylist(self, playlist, resume=True, handler=None, session_id=None):
        ...
        self.open()
        self._playVideo(resume and self.video.viewOffset.asInt(
        ) or 0, seeking=handler and handler.SEEK_PLAYLIST or 0, force_update=True)
    ...
    def createTrackListItem(self, track, fanart=None, index=0):
        ...
        li = xbmcgui.ListItem(
            track.title, path=url, thumbnailImage=track.defaultThumb.asTranscodedImageURL(800, 800))
        li.setInfo('music', {
    ...
    def onPlayBackStopped(self):
        ...
        Logger.debug('Player - STOPPED' +
                     (not self.started and ': FAILED' or ''))
        if not self.handler:
            return
    ...
    def onVideoWindowOpened(self):
        Logger.debug('Player: Video window opened')
        try:
    ...
    def onVideoWindowClosed(self):
        Logger.debug('Player: Video window closed')
        try:
    ...
    def onVideoOSD(self):
        Logger.debug('Player: Video OSD opened')
        try:
    ...
    def onSeekOSD(self):
        Logger.debug('Player: Seek OSD opened')
        try:
    ...
    def stopAndWait(self):
        if self.isPlaying():
            Logger.debug('Player: Stopping and waiting...')
            self.stop()
            while not Monitor.waitForAbort(0.1) and self.isPlaying():
                pass
            Monitor.waitForAbort(0.2)
            Logger.debug('Player: Stopping and waiting...Done')

    def monitor(self):
        if not self.thread or not self.thread.isAlive():
            self.thread = threading.Thread(
                target=self._monitor, name='PLAYER:MONITOR')
            self.thread.start()
        ...
        try:
            while not Monitor.abortRequested and not self._closed:
                if not self.isPlaying():
                    Logger.debug('Player: Idling...')

                while not self.isPlaying() and not Monitor.abortRequested and not self._closed:
                    Monitor.waitForAbort(0.1)

                if self.isPlayingVideo():
                    Logger.debug('Monitoring video...')
                    self._videoMonitor()
                elif self.isPlayingAudio():
                    Logger.debug('Monitoring audio...')
                    self._audioMonitor()
                elif self.isPlaying():
                    Logger.debug('Monitoring pre-play...')
    ...
    def _preplayMonitor(self):
        ...
        while self.isPlaying() and not self.isPlayingVideo() and not self.isPlayingAudio() and not Monitor.abortRequested and not self._closed:
            Monitor.waitForAbort(0.1)
    ...
    def _videoMonitor(self):
        ...
        ct = 0
        while self.isPlayingVideo() and not Monitor.abortRequested and not self._closed:
    ...
    def _audioMonitor(self):
        ...
        ct = 0
        while self.isPlayingAudio() and not Monitor.abortRequested and not self._closed:
            self.currentTime = self.getTime()
            Monitor.waitForAbort(0.1)
