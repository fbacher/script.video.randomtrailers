...
class AbstractMovie(BaseMovie):
    ...
    @classmethod
    def get_native_source(cls) -> str:
        if cls.__name__ == LibraryMovie.__name__:
            return MovieField.LIBRARY_SOURCE
        if cls.__name__ == TMDbMovie.__name__:
            return MovieField.TMDB_SOURCE
        if cls.__name__ == TFHMovie.__name__:
            return MovieField.TFH_SOURCE
        if cls.__name__ == ITunesMovie.__name__:
            return MovieField.ITUNES_SOURCE
        if cls.__name__ == FolderMovie.__name__:
            return MovieField.FOLDER_SOURCE
        else:
            cls._logger.warning(f'Unrecognized Movie class: {cls.__name__}')
            return None

    def get_source_id(self) -> str:
        """
        Gets the primary key for this movie appropriate for the source of the movie

        """
        source: str = self.get_source()
        if source == MovieField.LIBRARY_SOURCE:
            return str(self.get_library_id())
        elif source == MovieField.TMDB_SOURCE:
            return str(self.get_tmdb_id())
        elif source == MovieField.TFH_SOURCE:
            return str(self.get_tfh_id())
        elif source == MovieField.ITUNES_SOURCE:
            return str(self.get_itunes_id())
        else:
            raise ValueError(f'Invalid movie source: {source}')
